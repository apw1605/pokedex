        ...
        <div className='grid grid-flow-row grid-cols-5 gap-2 text-xs md:grid-cols-10'>
          {pokemonTypes.map((name) => (
            <button
              ...
              className={`rounded border p-2 shadow-sm hover:scale-105 transition-all hover:bg-slate-300 ${activeType === name ? 'bg-app-tertiary' : ''
                }`}
            >
              {name}
            </button>
...
export const getStaticProps = async (start: number) => {
  ...
  const pokemonListRaw = await api
    .get<PokemonQuery>(`https://pokeapi.co/api/v2/pokemon/?offset=${start}&limit=100`)
    .then((response) => {
